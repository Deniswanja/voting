...
$name2 = mysqli_real_escape_string($conn,$_POST['lastname']);
$name3 = mysqli_real_escape_string($conn,$_POST['username']);
$pass = mysqli_real_escape_string($conn,$_POST['password']);

$sq = mysqli_query($conn, 'SELECT username FROM loginusers WHERE username="'.$_POST['username'].'"');
...
$sql = mysqli_query($conn, 'INSERT INTO voters(firstname,lastname,username)
         VALUES("'.$_POST['firstname'].'","'.$_POST['lastname'].'","'.$_POST['username'].'")');
		 if (!$sql) { 
		 die (mysqli_error($con));
		 }
$sql2 = mysqli_query($conn, 'INSERT INTO loginusers(username,password)
